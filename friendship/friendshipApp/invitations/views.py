...
def home(request):
    ...
    if request.method == "POST":
        form = InvitationForm(data)
        if form.is_valid():
            # ... votre code existant pour sauvegarder l'invitation ...
            # Après avoir appelé le service d'authentification...
            user_status = AuthAPIClient.get_user_status(username)
            if not user_status.get('exists'):
                return JsonResponse({"error": "L'utilisateur n'existe pas ou n'est pas connecté."}, status=404)
            # Continuez avec le traitement si l'utilisateur existe et est connecté
            # ...
            return JsonResponse({"message": "L'invitation a été envoyée avec succès !"}, status=200)
        else:
            # Ici, assurez-vous que les erreurs sont également au format JSON
