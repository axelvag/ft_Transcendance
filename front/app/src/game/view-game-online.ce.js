...
class ViewGameOnline extends HTMLElement {
  ...
  async getPlayerProfile(playerId) {
    ...
    const profile = await fetch(`https://127.0.0.1:8001/accounts/get_user_profile/${playerId}`, {
      method: 'GET',
  ...
  async displayMatchup() {
    try {
      this.#game = await fetch(`https://127.0.0.1:8009/games/${this.getAttribute('game-id')}`).then(res => res.json());
