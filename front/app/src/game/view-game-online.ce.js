  ...
  renderGame() {
    ...
    if (endBtn) {
      endBtn.addEventListener('click', async () => {
        ...
        let player_left_score =  this.#user.id === this.#game.player_left_id ? 5 : 0;
        let player_right_score = this.#user.id === this.#game.player_right_id ? 5 : 0;
        await fetchWinnerMatch2(this.#user.id, player_left_score, player_right_score);
      });
    }
  }
