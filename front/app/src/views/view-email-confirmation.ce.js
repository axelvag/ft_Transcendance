...
class ViewEmailConfirmation extends HTMLElement {
  async connectedCallback() {
    ...
    const response1 = await fetch(`https://127.0.0.1:8001/accounts/is_user_active/${uidb64}/${token}`);
    ...
    if (data1.success) {
      ...
      const response = await fetch(`https://127.0.0.1:8001/accounts/activate/${uidb64}/${token}`);
    ...
    resendButton.addEventListener('click', async () => {
      ...
      const response = await fetch(`https://127.0.0.1:8001/accounts/resend_email_confirmation/${uidb64}`);
      const data = await response.json();
    });
  }
}
